...
def prepare_simulation():
    ...
    - Returns existing results directly if already prepared
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // optional, use LLM to generate profiles
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation already complete, skipping redundant generation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"Simulation {simulation_id} not yet prepared, starting prepare task")
        ...
        try:
            ...
            # Save entity count to state (for immediate frontend access)
            ...
            # Failure does not affect downstream flow; background task will retry
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Prepare task started. Use /api/simulation/prepare/status to check progress.",
...
def get_prepare_status():
    ...
    Request (JSON):
        {
            ...
            "simulation_id": "sim_xxxx"      // optional, simulation ID (to check completed prep)
        }
    ...
    try:
        ...
        if not task_id:
            if simulation_id:
                # simulation_id provided but not yet prepared
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task not found; if simulation_id provided, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        # Sort by created_at descending, return the latest
...
def get_simulation_history():
    ...
    Used for the home page historical projects display, returns a rich list
    including project name, description, and other details.
    ...
    try:
        ...
        # Enrich simulation data from Simulation files only
        ...
        for sim in simulations:
            ...
            # Get associated project file list (up to 3 files)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profiles for a simulation in real-time (for viewing progress during generation).
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (complete config generated intelligently by LLM).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // optional: dynamically update Agent activity to Zep graph memory
            "force": false                         // optional: force restart (stops running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, if the simulation is running or completed, it stops and clears run logs first
        - Cleared content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Does NOT clear config files (simulation_config.json) or profile files
        - Useful for re-running a simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activity (posts, comments, likes, etc.) is updated to the Zep graph in real-time
        - This allows the graph to "remember" the simulation for subsequent analysis or AI conversation
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete, check if a process is running
                if state.status == SimulationStatus.RUNNING:
                    # Check if the simulation process is actually running
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running. Call /stop to stop it, or use force=true to force restart."
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows both platforms' content for the current latest round
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for frontend progress bar and timeline views.
...
def interview_agent():
    ...
    (after completing simulation rounds and entering command-waiting mode).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // required, simulation ID
            "agent_id": 0,                     // required, Agent ID
            "prompt": "What is your opinion?", // required, interview question
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // omit platform to use default
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden per item)
                                               // if not specified: dual-platform interviews each Agent on both platforms
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // optional, platform type (reddit/twitter)
                                           // if not specified, return history for both platforms
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Environment is running and accepting Interview commands"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    exit command-waiting mode.

    Note: This is different from the /stop endpoint. /stop forcefully terminates
