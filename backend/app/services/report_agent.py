...
class ReportLogger:
    ...
    def log_tool_result(
        ...
        """Log a tool invocation result (full content, not truncated)."""
        self.log(
            action="tool_result",
            stage="generating",
            section_title=section_title,
    ...
    def log_section_content(
        ...
        """Log section content generation completion (content only, does not mean the entire section is finished)."""
        self.log(
    ...
    def log_section_full_complete(
        ...
        has truly finished and to obtain its full content.
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    inside the report folder. These logs differ from agent_log.jsonl in that
...
[Return Content]
- Relevant fact excerpts (can be quoted directly)
...
This is not an LLM simulation, but calls the real interview interface to obtain simulation Agents' original responses.
...
[Use Cases]
- Need to understand event perspectives from different roles (What do students think? What does media say? What is the official position?)
...
You are an expert at writing "Future Prediction Reports" for a specific section of the report.
...
and the behavior and interactions of Agents in the simulation are predictions of future population behavior.
...
❌ Do NOT write as an analysis of the current real world
✅ Focus on "what the future holds" — simulation results are predicted futures
...
1. [MUST USE TOOLS TO OBSERVE THE SIMULATED WORLD]
   ...
   - All content must come from events and Agent behaviors within the simulated world
...
2. [MUST QUOTE AGENTS' ORIGINAL STATEMENTS AND BEHAVIORS]
   ...
   - Use quote format in the report to present these predictions, for example:
     > "A certain population will say: original content..."
...
3. [LANGUAGE CONSISTENCY - QUOTED CONTENT MUST BE TRANSLATED TO REPORT LANGUAGE]
   - Tool results may contain English or mixed English/Chinese expressions
   - If the simulation requirement and source material are in Chinese, the report must be written entirely in Chinese
   - When quoting English or mixed English/Chinese content from tools, translate it to fluent Chinese before writing it into the report
   - Preserve the original meaning; ensure the expression is natural and coherent
...
4. [FAITHFULLY PRESENT PREDICTION RESULTS]
   ...
   - If information on a particular aspect is insufficient, state so honestly
...
- Each section is the minimum subdivision unit of the report
- ❌ Do NOT use any Markdown headings (#, ##, ###, #### etc.) within a section
- ❌ Do NOT add the section heading at the start of the content
...
Weibo served as the primary venue for public opinion, handling the core function of first-break news:

> "Weibo contributed 68% of first-break volume..."
...
### I. Initial Phase     ← Wrong! Do not use ### subsections
...
Each reply you can only do ONE of the following (not both at once):
...
When you have gathered enough information through tools, begin your output with "Final Answer:".
...
4. [QUOTE FORMAT — MUST BE IN ITS OWN PARAGRAPH]
   Blockquotes must stand alone with a blank line before and after; do not embed them inside a paragraph:

   ✅ Correct format:
   ```
   The school's response was seen as lacking substance.

   > "The school's crisis response style appeared rigid and slow in the fast-moving social media environment."
   ...
   ❌ Incorrect format:
   ```
   The school's response was seen as lacking substance. > "The school's crisis response style..." This assessment reflects...
   ```
...
3. After gathering enough information, output Final Answer (body text only, no headings at all)"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "Please call more tools to gather simulation data before outputting a Final Answer. {unused_hint}"
)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately output section content starting with "Final Answer:" based on the information gathered.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used yet: {unused_list}. Try different tools for multi-perspective information."
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON directly (without <tool_call> tag)
        # Only attempt when format 1 didn't match, to avoid false positives in body text JSON
    ...
    def _is_valid_tool_call(self, data: dict) -> bool:
        ...
        # Supports both {"name": ..., "parameters": ...} and {"tool": ..., "params": ...} key schemas
    ...
    def plan_outline(
        ...
        Uses the LLM to analyze simulation requirements and plan the report's section structure.
        ...
        try:
            ...
            return ReportOutline(
                ...
                sections=[
                    ...
                    ReportSection(title="Future Trends and Risk Outlook")
                ]
            )
    ...
    def _generate_section_react(
        ...
        # Build user prompt — each completed section is truncated to 4000 chars
        if previous_sections:
            previous_parts = []
            for sec in previous_sections:
                # each section truncated to max 4000 chars
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Final iteration also returned None; break out to forced finish
                break
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = f"(These tools haven't been used yet; consider using them: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",
            ...
            # Adopt the LLM output directly as the final answer; no more iterations
    ...
    def generate_report(
        ...
        """
        Generate a complete report (outputting sections in real time).

        Each section is saved to the folder immediately after generation, without
        waiting for the entire report to complete.
        File structure:
        reports/{report_id}/
        ...
        try:
            ...
            ReportManager.update_progress(
                self.tenant_id, report_id, "pending", 0, "Initializing report...",
                completed_sections=[]
            )
            ...
            ReportManager.update_progress(
                self.tenant_id, report_id, "planning", 5, "Starting report outline planning...",
                completed_sections=[]
            )
            ...
            ReportManager.update_progress(
                self.tenant_id, report_id, "planning", 15, f"Outline planning complete, {len(outline.sections)} sections",
                completed_sections=[]
            )
            ReportManager.save_report(self.tenant_id, report)
            ...
            # Phase 2: Generate section by section (save incrementally)
            ...
            ReportManager.update_progress(
                self.tenant_id, report_id, "generating", 95, "Assembling full report...",
                completed_sections=completed_section_titles
            )
            ...
            ReportManager.save_report(self.tenant_id, report)
            ReportManager.update_progress(
                self.tenant_id, report_id, "completed", 100, "Report generation complete",
                completed_sections=completed_section_titles
            )
            ...
            try:
                ReportManager.save_report(self.tenant_id, report)
                ReportManager.update_progress(
                    self.tenant_id, report_id, "failed", -1, f"Report generation failed: {str(e)}",
                    completed_sections=completed_section_titles
                )
            except Exception:
...
class ReportManager:
    ...
    def get_console_log(cls, tenant_id: str, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        These are plain-text console output logs (INFO, WARNING, etc.) generated
        during report generation, distinct from the structured agent_log.jsonl logs.
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep original log lines; strip trailing newline
    ...
    def save_section(
        ...
        Called immediately after each section is generated for incremental output.
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove duplicate Markdown heading lines at the start that match the section title.
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this heading duplicates the section title (skip within first 5 lines)
                if i < 5:
                ...
                # Section headings are added by the system; content must contain no headings
    ...
    def assemble_full_report(cls, tenant_id: str, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-process: clean heading issues in the full report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report main heading (#) and section headings (##); convert deeper headings (###, ####, etc.) to bold.
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (same title within the last 5 lines)
                is_duplicate = False
                ...
                if level == 1:
                    if title == outline.title:  # keep report main heading
                        processed_lines.append(line)
                        prev_was_heading = True
                    elif title in section_titles:  # section title incorrectly used #; fix to ##
                    ...
                    if title in section_titles or title == outline.title:  # keep section headings
                        processed_lines.append(line)
                        prev_was_heading = True
                    else:
                    ...
                    # Convert ### and below to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
                ...
                # skip horizontal rule immediately after heading
                i += 1
                continue
