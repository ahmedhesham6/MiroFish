...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep graph
        2. Generate an OASIS Agent Profile for each entity (optional LLM enhancement, supports parallel execution)
        3. Use an LLM to intelligently generate simulation config parameters (timing, activity level, posting frequency, etc.)
        4. Save the config file and profile files
        ...
        Args:
            ...
            document_text: Raw document content (used by LLM to understand background)
            defined_entity_types: Pre-defined entity types (optional)
            use_llm_for_profiles: Whether to use the LLM to generate detailed agent personas
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No matching entities found — please verify the graph was built correctly"
                self._save_simulation_state(state)
                return state
            ...
            # Reddit profiles are already saved in realtime during generation; save again here to ensure completeness
            if progress_callback:
                progress_callback(
