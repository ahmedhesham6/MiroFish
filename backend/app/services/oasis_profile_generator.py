...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # Add additional persona fields if present
        if self.age:
...
class OasisProfileGenerator:
    ...
    2. Generates highly detailed personas (including background, career, personality, social media behavior, etc.)
    3. Distinguishes between individual entities and abstract group entities
    """
    ...
    # Individual entity types (require concrete persona generation)
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Retrieve rich information about an entity using Zep graph hybrid search.

        Zep has no built-in hybrid search endpoint, so edges and nodes are
        searched separately and the results are merged. Parallel requests are
        used for efficiency.
        ...
        if not self.graph_id:
            logger.debug("Skipping Zep retrieval: graph_id is not set")
            return results
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Enriched information from Zep hybrid retrieval
        """
        context_parts = []
        ...
        # 3. Detailed information from related nodes
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: generate a concrete personal profile
        - Group/institutional entity: generate a representative account profile
        """
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last character is not a closing delimiter,
        # the string value was probably cut off mid-way
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First try to fix truncated JSON
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # 3. Fix literal newlines inside string values
        ...
        # 6. Extract partial information from raw content
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = (
            "You are an expert social media user persona generator. "
            "Generate detailed, realistic personas for public opinion simulation, "
            "reproducing real-world conditions as accurately as possible. "
            "You MUST return valid JSON. No unescaped newlines inside string values."
        )
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity below, reproducing real-world conditions as accurately as possible.
...
2. persona: Detailed persona description (~2000 words, plain text), covering:
   ...
   - Personal background (key experiences, connection to the event, social relationships)
   - Personality (MBTI type, core traits, emotional expression style)
   - Social media behavior (posting frequency, content preferences, interaction style, writing style)
   - Stances and opinions (attitude toward topics, content likely to provoke or move them)
   - Distinctive traits (catchphrases, unique experiences, hobbies)
   - Personal memory (important part of the persona: relationship with the event and existing actions/reactions)
...
Important:
- All field values must be strings or numbers — no literal newlines
- persona must be a single continuous prose paragraph
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build a detailed persona prompt for a group or institutional entity."""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media account profile for the institution/group entity below, reproducing real-world conditions as accurately as possible.
...
2. persona: Detailed account profile (~2000 words, plain text), covering:
   - Basic institutional info (formal name, nature, founding background, main functions)
   - Account positioning (account type, target audience, core purpose)
   - Communication style (language characteristics, common expressions, forbidden topics)
   - Content characteristics (content types, posting frequency, active time slots)
   - Official stances (position on key topics, approach to controversy)
   - Special notes (represented group profile, operational habits)
   - Institutional memory (important part of the persona: relationship with the event and existing actions/reactions)
...
Important:
- All field values must be strings or numbers — no null values, no literal newlines
- persona must be a single continuous prose paragraph
- age must be integer 30; gender must be string "other"
- Account communication must match the institution's identity"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for Zep retrieval (richer context)
            parallel_count: Number of parallel workers (default 5)
            realtime_output_path: If provided, write profiles to this file after each generation
        ...
        def save_profiles_realtime():
            """Write already-generated profiles to file."""
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            """Worker function: generate a single profile."""
            entity_type = entity.get_entity_type() or "Entity"
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write fallback profile to file in real time
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print the generated persona to the console in full (no truncation)."""
        separator = "-" * 70
        ...
        # Print to console only (avoids duplication — logger no longer emits the full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to a file using the correct format for the platform.
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - description: Short public bio (visible on profile pages)

        user_char vs description:
        - user_char: Internal — the LLM system prompt that determines how the agent thinks and acts
        - description: External — the bio visible to other users
        """
        import csv
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Newlines are not safe in CSV — replace with spaces
                ...
                row = [
                    ...
                    profile.name,           # name: real display name
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses the same format as to_reddit_format() so OASIS can read them correctly.
        The user_id field is critical — it is the key used by OASIS agent_graph.get_agent()
        to match profiles against poster_agent_id in initial_posts.

        Required fields:
        - user_id: Integer user ID (matches poster_agent_id in initial_posts)
        - username: Username
        - name: Display name
        - bio: Short bio
        ...
        for idx, profile in enumerate(profiles):
            item = {
                ...
                # OASIS required fields — always include defaults
