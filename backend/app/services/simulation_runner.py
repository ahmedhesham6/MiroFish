...
class SimulationRunState:
    ...
    # Per-platform round and simulated time (for dual-platform parallel display)
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to update Agent activities to the Zep graph dynamically.
        ...
        try:
            ...
            # Add --max-rounds flag if specified
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])

            # Create main log file to avoid blocking from full stdout/stderr pipe buffer
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Set subprocess env vars to ensure UTF-8 encoding on Windows
            # Fixes encoding issues in third-party libs (e.g. OASIS) that don't specify encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ — makes all open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # ensure stdout/stderr use UTF-8

            # Set working directory to sim dir (db and other files are generated there)
            # Use start_new_session=True to create a new process group; allows os.killpg to terminate all child processes
            process = subprocess.Popen(
                ...
                bufsize=1,
                env=env,  # pass env vars with UTF-8 settings
                start_new_session=True,  # new process group; ensures all related processes are terminated on server shutdown
            )
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end event and mark platform complete
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform ran, check only that one.
                                    # If both platforms ran, both must be complete.
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update per-platform round and time
                                    if platform == "twitter":
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check whether all enabled platforms have completed their simulations.

        Determines whether a platform is enabled by checking whether its
        corresponding actions.jsonl file exists.
        ...
        # Return False if a platform is enabled but not yet complete
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # Unix: terminate by process group
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when the action record lacks a platform field).
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record's platform field; fall back to default_platform
    ...
    def get_all_actions(
        ...
        # Read Reddit action file (platform auto-set to reddit based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files don't exist, try legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # legacy format files should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing a fresh simulation restart).
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean (avoid printing unnecessary logs for empty process lists)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)

        if not has_processes and not has_updaters:
            return  # nothing to clean; exit silently
    ...
    def register_cleanup(cls):
        ...
        Call during Flask app startup to ensure all simulation processes are
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (the one actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader subprocess
        # In non-debug mode this env var is absent; registration is still needed
        ...
        # SIGHUP only exists on Unix (macOS/Linux); not available on Windows
        ...
        try:
            ...
            # Not in main thread; atexit is the only option
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional).
                ...
                - None: interview both platforms in a dual-platform simulation and return merged result
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment not running or already shut down; cannot execute interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden per interview item).
                ...
                - None: each Agent interviewed on both platforms in dual-platform simulations
            timeout: Timeout in seconds.
    ...
    def interview_all_agents(
        ...
        Uses the same question to interview every Agent in the simulation.
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation so it exits the
        ...
        try:
            ...
            # Timeout may mean the environment is in the process of shutting down
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specific Agent ID (optional; only retrieve history for that Agent).
        ...
        # If multiple platforms queried, cap the total
