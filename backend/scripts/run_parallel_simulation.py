...
- Keeps the environment alive after simulation completes, entering command-wait mode
...
# This addresses OASIS third-party library file reads that omit an explicit encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Force set default encoding (affects the default encoding of open())
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        # Only set default encoding for text mode (non-binary) when no encoding is specified
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (we intentionally omit max_tokens to let the model decide)"""
...
# Action type mapping (database names -> standard names)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to display real entity names instead of codes like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and enrich with full context information

    Args:
        db_path: Database file path
        last_rowid: Last read maximum rowid value (using rowid instead of created_at because created_at formats differ between platforms)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (preserving full content, no truncation)
            simplified_args = {}
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, parallel simulation can use different API providers for different platforms to increase concurrency.
    ...
    # If .env has no model name, use config as fallback
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row in the database (using rowid to avoid created_at format differences)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Shut down the environment immediately after simulation, do not enter command-wait mode'
    )

    args = parser.parse_args()

    # Create the shutdown event at the beginning of main to ensure the entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure graceful exit when SIGTERM/SIGINT is received

    Persistent simulation scenario: the simulation does not exit after completion, waiting for interview commands.
    When a termination signal is received, we need to:
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify the asyncio loop to exit (giving the loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
