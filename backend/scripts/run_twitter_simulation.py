...
Reads parameters from a config file to run the simulation end-to-end automatically.
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Uses config from the project root .env file (highest priority):
        - LLM_API_KEY: API key
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability per agent config
        candidates = []
        for cfg in agent_configs:
            ...
            # Activate based on activity level probability
    ...
    async def run(self, max_rounds: int = None):
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command-wait mode - environment remains running")
...
async def main():
    ...
    # Create the shutdown event at the beginning of main
...
def setup_signal_handlers():
    ...
    Gives the program a chance to clean up resources (close databases, environments, etc.).
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only if signal received repeatedly
            print("Force exiting...")
            sys.exit(1)
