            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but the full section may not yet be complete) -->
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **alumni_345（index=1）**: as a Wuhan University alumnus...
        ...
        // e.g.: - Select parent_601（index 0）: as a representative of the parent group...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:选择|Select\s+)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)  // 选择 = "Select" (legacy Chinese format)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // e.g.: - **parent_601（index 0）**: as a representative of the parent group...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择|^Not selected|^In summary|^Final selection/)) {  // 未选 = "Not selected", 综上 = "In summary", 最终选择 = "Final selection" (legacy Chinese markers)
          // Continuation of the reason (exclude trailing summary paragraphs)
    ...
    const interviewBlocks = text.split(/#### (?:采访|Interview) #\d+:/).slice(1)  // 采访 = "Interview" (legacy Chinese section header)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if the first question starts with "1."
      ...
      // Extract answers - separated into Twitter and Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:关键引言|Key Quotes)|$)/)  // 关键引言 = "Key Quotes" (legacy Chinese label)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/【(?:Twitter平台回答|Twitter Answer)】\n?([\s\S]*?)(?=【(?:Reddit平台回答|Reddit Answer)】|$)/)  // Twitter平台回答 = "Twitter Answer", Reddit平台回答 = "Reddit Answer" (legacy Chinese labels)
        const redditMatch = answerText.match(/【(?:Reddit平台回答|Reddit Answer)】\n?([\s\S]*?)$/)
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer: copy as default display only if not a placeholder
    ...
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary and Key Insights)\n([\s\S]*?)$/)  // 采访摘要与核心观点 = "Interview Summary and Key Insights" (legacy Chinese section header)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const edgesSection = text.match(/### (?:相关边|Related Edges):\n([\s\S]*?)(?=\n###|$)/)  // 相关边 = "Related Edges" (legacy Chinese section header)
    if (edgesSection) {
    ...
    const nodesSection = text.match(/### (?:相关节点|Related Nodes):\n([\s\S]*?)(?=\n###|$)/)  // 相关节点 = "Related Nodes" (legacy Chinese section header)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Predicted Scenario: '),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Get current platform selection for a given question
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Set platform selection for a given question
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' ||  // legacy Chinese: "No reply from this platform" / "No reply"
             t === '(No reply from this platform)' || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" or "问题X：" (new backend format; 问题 = "Question" in legacy Chinese)
      // 2. "1. " or "\n1. " (number+dot, legacy format compatibility)
      let matches = []
      let match

      // Prefer "Question X:" / "问题X：" format first (问题 = "Question", legacy Chinese)
      const cnPattern = /(?:^|[\r\n]+)(?:问题|Question\s*)(\d+)[：:]\s*/g
      ...
      // If no numbering found or only one found, return the full text
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^(?:问题|Question\s*)\d+[：:]\s*/, '')  // 问题 = "Question" (legacy Chinese prefix)
          .replace(/^\d+\.\s+/, '')
          .trim()
        return [cleaned || answerText]
      }

      // Extract each part by index
      const parts = []
    ...
    // Get the answer corresponding to a given question index
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for the first question, empty for the rest
      return qIdx === 0 ? answer : ''
    }

    // Check whether a given question has dual-platform answers (filtering out placeholders)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (only shown when there are real dual-platform answers)
...
// Currently active step (shown in the header)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading h2 heading (## xxx) since the section title is already shown in the outer layer
  ...
  // Remove <br> immediately after block-level elements before paragraph start tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: maintain incrementing counter when single-item <ol> is separated by paragraph content
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Auto-expand the section that was just generated
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in a single nextTick after the loop ends
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is done, scroll to the top; otherwise scroll to the bottom to follow the latest log
            if (isComplete.value) {
...
// Extract final answer content - extracts section content from LLM response
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" (legacy Chinese final answer marker; 最终答案 = "Final Answer")
  ...
  // If content is long and contains markdown formatting, try removing the thought process before returning
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove leading "Thought:" section
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* dark grey, does not change with state */
  font-weight: 500;
}
