    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a new simulation instance and load world parameter templates
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically call tools that extract entities and relationships from the knowledge graph, initialize simulated individuals, and assign them unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Real-world Seed Topic Count</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow speed, recommendation algorithms, active time slots, posting frequency, event triggers, and other parameters for each individual based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generates initial activation events and trending topics based on the narrative direction to guide the simulation world's initial state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Config</span>
                <span class="section-desc">MiroFish automatically plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first runs, it is strongly recommended to switch to 'Custom Mode' to reduce simulation rounds for a quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// Log dedup: record the last logged key info
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entered config generation phase, begin polling for config
    if (!configTimer) {
...
// Calculate auto-generated rounds from config (do not use hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // return null when config has not been generated yet
  }
  ...
  // Ensure max rounds is at least 40 (recommended), to avoid abnormal slider range
...
// Get the username corresponding to the given agent_id
...
// Calculate total number of interested topics across all profiles
...
const handleStartSimulation = () => {
  // Build params to pass to the parent component
  const params = {}

  if (useCustomRounds.value) {
    // User-defined rounds: pass max_rounds param
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation, custom rounds: ${customMaxRounds.value}`)
  } else {
    // User chose to keep the auto-generated round count, no max_rounds param passed
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Previously completed preparation detected, reusing it')
      ...
      startPolling()
      // Begin real-time profile fetching
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when API returns a valid value, to avoid overwriting an existing valid value
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        // If all profiles have been generated
