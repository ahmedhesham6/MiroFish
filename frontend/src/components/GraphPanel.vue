...
// Toggle expanded/collapsed state for a self-loop item
...
const renderGraph = () => {
  ...
  // Process edge data: count and index edges between the same node pair
  const edgePairCount = {}
  ...
  // Track how many edges have been processed for each node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // nodes whose self-loops have been processed
  ...
  tempEdges.forEach(e => {
    ...
    // Determine whether the edge direction matches the canonical direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Calculate curvature: spread multiple edges apart; single edges are straight
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly to ensure clear separation
      // Curvature range grows with the number of edges
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // Flip curvature when edge direction is opposite to canonical direction
      // This ensures all edges are distributed in the same reference frame without overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, +40 per extra edge
    ...
    // Add gravity toward center so isolated node clusters gather in the center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that leaves and returns to the same node
      const loopRadius = 30
    ...
    // Compute curve control point — dynamically adjusted based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset proportional to distance, ensuring the curve is visually distinct
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Midpoint of quadratic Bézier curve at t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Quadratic Bézier formula: B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Link labels background (white background makes text more legible)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether a real drag has started (movement exceeds threshold)
      ...
      .on('end', (event, d) => {
        // Let the simulation cool down only if an actual drag occurred
        if (d._isDragging) {
