...
// Reset all state (used for restarting a simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid carryover from previous simulation
  resetAllState()
...
// Track the last round number per platform to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is complete (via runner_status or platform completion state)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: if backend hasn't updated runner_status yet but platforms report completion
      // Detect via twitter_completed and reddit_completed
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform is complete, check whether all enabled platforms are done
  // Determine if a platform is enabled via actions_count (> 0) or running flag
  ...
  // Check that all enabled platforms have completed
