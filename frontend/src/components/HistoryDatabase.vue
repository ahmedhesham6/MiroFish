...
let pendingState = null  // stores the target state to execute
...
const containerStyle = computed(() => {
  ...
  // Compute required height: rows * cardHeight + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute card count in current row to center each row
    ...
    // Expand downward with extra spacing from the title
    ...
    const x = offset * 35
    // Adjust start position: near title but with appropriate spacing
...
// Get progress CSS class based on round count
...
// Format round count display (current / total)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Record the target state (always update regardless of animation state)
        pendingState = shouldExpand

        // Clear the previous debounce timer (new scroll intent overrides old)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If an animation is in progress, only record the state and handle it after animation ends
        if (isAnimating) return

        // If target state equals current state, nothing to do
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce to delay state transition and prevent rapid flickering
        // Shorter delay (50ms) for expand, longer (200ms) for collapse to improve stability
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Check whether an animation is in progress
          if (isAnimating) return

          // Check whether the pending state still needs to be applied (may have been overridden by a later scroll)
          ...
          // Release lock after animation; check for any pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation, check whether there is a new pending state
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink viewport bottom upward, requiring more scrolling before expand triggers
...
onMounted(async () => {
  ...
  // Wait for DOM render before initializing observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* CSS background pattern creating a fixed-interval square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor to top-left; height changes only expand at the bottom, not shifting existing grid */
...
.cards-container {
  ...
  /* min-height computed dynamically by JS based on card count */
}
