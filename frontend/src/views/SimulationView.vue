 ...
 * Called when the user returns from Step 3 to Step 2, assuming they want to exit the simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Attempt graceful close of simulation environment
      try {
        ...
        // If graceful close threw, try force stop
        await forceStopSimulation()
      }
    } else {
      // Environment not running, but process may still be alive; check simulation status
      const simRes = await getSimulation(currentSimulationId.value)
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Detected simulation status is running, stopping...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // Failure to check env status does not block the flow
...
onMounted(async () => {
  ...
  // Check and close any running simulation (user returning from Step 3)
